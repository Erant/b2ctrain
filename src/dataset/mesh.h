...
struct TriMesh {
  ...
  std::vector<uint8_t> alpha; // [nv], or empty: mesh-bake's cap coverage (255 = the colour came from the projected photo)
  ...
  bool has_alpha() const { return alpha.size() == nv(); }
...
// Binary little-endian PLY: float x/y/z [nx/ny/nz] [uchar red/green/blue [alpha]], faces as `list uchar uint vertex_indices`.
