...
// One block per tile, all three channels. Computes the loss map contribution and the per-pixel gradient.
constexpr size_t PH_SMEM = sizeof(float) * (3 * 2 * IN_W * IN_W + IN_W * STAT_W * 5 + STAT_W * STAT_W * 5);
...
__global__ void __launch_bounds__(TILE_PX) photometric_kernel(
  ...
  extern __shared__ __align__(16) float ph_smem[];
  float* s_in = ph_smem;                                    // [3][2][IN_W*IN_W]: pred, gt_eff per channel
  float* s_h = s_in + 3 * 2 * IN_W * IN_W;                  // horizontal pass (rows 0..35, cols 0..25)
  float* s_v = s_h + IN_W * STAT_W * 5;                     // vertical pass / partial maps
  ...
  // Load the 36x36 footprint for all channels (zero outside the image).
  for (int i = tid; i < IN_W * IN_W; i += TILE_PX) {
    int ly = i / IN_W, lx = i % IN_W;
    int gy = ty0 + ly - 2 * HALO, gx = tx0 + lx - 2 * HALO;
    float p[3] = {0.f, 0.f, 0.f}, g[3] = {0.f, 0.f, 0.f};
    if (gy >= 0 && gy < H && gx >= 0 && gx < W) {
      int pix = gy * W + gx;
      float4 pv = pred[pix];
      p[0] = pv.x; p[1] = pv.y; p[2] = pv.z;
      uint32_t gp = gt[pix];
      float one_minus_a = lp.composite ? (1.f - gt_channel(gp, 3)) : 0.f;
      for (int c = 0; c < 3; c++) g[c] = gt_channel(gp, c) + one_minus_a * lp.bg[c];
    }
    for (int c = 0; c < 3; c++) { s_in[(c * 2) * IN_W * IN_W + i] = p[c]; s_in[(c * 2 + 1) * IN_W * IN_W + i] = g[c]; }
  }
  const float norm = lp.scale / (3.f * (float)W * (float)H);
  float loss_local = 0.f;
  for (int c = 0; c < 3; c++) {
    const float* sp = s_in + (c * 2) * IN_W * IN_W;
    const float* sg = sp + IN_W * IN_W;
    __syncthreads();
    // Horizontal blur: rows 0..35, output cols 0..25.
...
#pragma unroll
      for (int d = 0; d < 11; d++) {
        float w = c_gauss[d];
        float x = sp[ly * IN_W + ox + d], y = sg[ly * IN_W + ox + d];
        sx += w * x; sx2 += w * x * x; sy += w * y; sy2 += w * y * y; sxy += w * x * y;
      }
      float* o = s_h + i * 5; o[0] = sx; o[1] = sx2; o[2] = sy; o[3] = sy2; o[4] = sxy;
    }
    __syncthreads();
    // Vertical blur -> stats at 26x26, SSIM, partial maps A, B, Bmu1, C, Cmu2 (scaled by M_p * ssim_w).
...
#pragma unroll
      ...
      if (gy >= 0 && gy < H && gx >= 0 && gx < W) {
        ...
        float p = sp[(oy + HALO) * IN_W + ox + HALO], g = sg[(oy + HALO) * IN_W + ox + HALO];
...
#pragma unroll
        ...
        int pix = gy * W + gx;
        float p = sp[(ly + 2 * HALO) * IN_W + lx + 2 * HALO], g = sg[(ly + 2 * HALO) * IN_W + lx + 2 * HALO];
        uint32_t gp = gt[pix];
        float gt_a = gt_channel(gp, 3);
        float wp = weights ? weights[pix] * (1.f / 255.f) : 1.f;
        float M = wp * (lp.mask ? gt_a : 1.f) * norm;
...
void photometric_loss(RenderCtx& ctx, const ViewGPU& view, const LossParams& lp, cudaStream_t stream) {
  ...
  if (!attr) { CUDA_CHECK(cudaFuncSetAttribute(photometric_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)PH_SMEM)); attr = true; }
  dim3 grid(loss_tiles(ctx)), block(TILE_PX);
  photometric_kernel<<<grid, block, PH_SMEM, stream>>>(ctx.out_rgba, view.rgba, view.weights, ctx.W, ctx.H, loss_tiles_x(ctx), lp, (float*)ctx.v_out.ptr, ctx.loss_accum.ptr);
  CUDA_KERNEL_CHECK();
}
