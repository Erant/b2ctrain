...
template <int K3>
__global__ void prune_flags_kernel(int n, const float4* __restrict__ pos, const float4* __restrict__ quat, const float4* __restrict__ ls, const float* __restrict__ sh,
  ...
  const float* s = sh + (size_t)i * K3;
  for (int k = 0; k < K3; k++) bad |= !isfinite(s[k]);
  if (!bad) {
    float o = eff_opacity(p.w, l);
...
__global__ void split_kernel(int count, const uint32_t* __restrict__ parents, const uint32_t* __restrict__ child_slots,
                             ...
                             float* __restrict__ m_sh, float* __restrict__ v_sh, float* __restrict__ refine_norm, float* __restrict__ max_screen, float* __restrict__ vis_count,
  ...
  quat[c] = qn; quat[p] = qn;
  ls[p] = nl; ls[c] = nl;
  const float* sp = sh + (size_t)p * K3; float* sc = sh + (size_t)c * K3;
  for (int j = 0; j < K3; j++) sc[j] = sp[j];
  ...
  float* mp = m_sh + (size_t)p * K3; float* mc = m_sh + (size_t)c * K3;
  for (int j = 0; j < K3; j++) { mp[j] = 0.f; mc[j] = 0.f; }
...
#define SK(K) split_kernel<K><<<div_up(n_sel, 256), 256, 0, stream>>>(n_sel, sel_parent, sel_child, m.pos_op, m.quat, m.lscale, m.sh, m.m_pos_op, m.v_pos_op, m.m_quat, m.v_quat, m.m_lscale, m.v_lscale, m.m_sh, m.v_sh, m.refine_norm, m.max_screen, m.vis_count, ctx.tile_count, p.split_at_screen_size)
