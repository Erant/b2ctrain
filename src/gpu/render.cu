...
void RenderCtx::setup(int w, int h, int n, cudaStream_t stream) {
  ...
  out_rgba.reserve(npx); out_feat.reserve(npx); hollow_pen.reserve(npx); hollow_zfirst.reserve(npx); hollow_zpush.reserve(npx); last_idx.reserve(npx); v_out.reserve(npx); v_feat.reserve(npx);
