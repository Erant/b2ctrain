...
__global__ void project_kernel(int n, const float4* __restrict__ pos_op, const float4* __restrict__ quat, const float4* __restrict__ lscale,
  ...
  float3 col = sh_eval<DEG>(sh + (size_t)i * K * 3, v, active_deg);
...
void launch_deg(RenderCtx& ctx, const Model& m, const RenderParams& p, const CamDev& cam, cudaStream_t stream) {
  ...
  switch (p.feat) {
    case FeatureMode::None:
      project_kernel<DEG, 0><<<blocks, PROJ_BLOCK, 0, stream>>>(m.n, m.pos_op, m.quat, m.lscale, m.sh, nullptr, cam, p.mip, ctx.tiles_x, ctx.tiles_y, ctx.proj0, ctx.proj1, ctx.proj2, ctx.proj3, ctx.tile_count, ms, p.sh_degree); break;
    case FeatureMode::Normals:
      project_kernel<DEG, 1><<<blocks, PROJ_BLOCK, 0, stream>>>(m.n, m.pos_op, m.quat, m.lscale, m.sh, nullptr, cam, p.mip, ctx.tiles_x, ctx.tiles_y, ctx.proj0, ctx.proj1, ctx.proj2, ctx.proj3, ctx.tile_count, ms, p.sh_degree); break;
    case FeatureMode::Buffer:
      project_kernel<DEG, 2><<<blocks, PROJ_BLOCK, 0, stream>>>(m.n, m.pos_op, m.quat, m.lscale, m.sh, p.feat_buffer, cam, p.mip, ctx.tiles_x, ctx.tiles_y, ctx.proj0, ctx.proj1, ctx.proj2, ctx.proj3, ctx.tile_count, ms, p.sh_degree); break;
  }
  CUDA_KERNEL_CHECK();
}
